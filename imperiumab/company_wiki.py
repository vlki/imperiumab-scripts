...
from imperiumab.wiki import parse_wiki_date
...
def get_company_page_names(wiki):
    ...
    company_pages = filter(lambda company_page: company_page.name not in not_companies_pages,
                           company_category.members())
...
def get_company_page_url(wiki, company_page_name):
    company_structured_data = wiki.site.raw_api(action='browsebysubject', subject=company_page_name)

    url_company_name = company_structured_data['query']['subject'].split('#')[0]

    return wiki.base_url + urllib.parse.quote(url_company_name)
